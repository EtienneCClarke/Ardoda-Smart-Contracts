...
contract Kollab_Share_Factory {
    ...
    mapping(address => uint[]) public created_splitters;
    uint idTracker = 100;
    ...
    function createSplitter (
        ...
        splitters[_id] = new Kollab_Share(_payees, _shares, address(this), _name, _desc, msg.sender, withdrawlFee, owner);
        for(uint i = 0; i < _payees.length; i++) { associated_splitters[_payees[i]].push(_id); }
        created_splitters[msg.sender].push(_id);
    ...
    function getSplitter (uint _id, address _account) public view returns (string[] memory) {
        ...
        if(!this.checkExists(_id)) { return result; }
    ...
    // Gets all splitter ids associated with account
    function getSplitterIds(address _account) public view returns (uint[] memory) {
        return associated_splitters[_account];
    }

    function getCreatedSplitterIds(address _account) public view returns (uint[] memory) {
        return created_splitters[_account];
    }
    ...
    function changeOwner(address payable _newOwner) onlyOwner public {
        for(uint i = 101; i <= idTracker; i++) {
            splitters[i].changeOwner(_newOwner);
        }
        owner = _newOwner;
    }
...
contract Kollab_Share {
    ...
    constructor(address[] memory _payees, uint[] memory _shares, address _factory, string memory _name, string memory _desc, address _creator, uint _fee, address _owner) {
        payees = _payees;
        creator = _creator;
        name = _name;
        description = _desc;
    ...
    modifier onlyOwner {
        require(msg.sender == owner, 'Only the owner has access to this functionality.');
        _;
    }
    ...
    // Get shareholders of this contract
