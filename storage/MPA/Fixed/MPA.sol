...
contract MPA {
    ...
    event withdrawal(uint amount, address indexed to);
    event deposit(uint amount, address indexed from);
    ...
    function getShareholders() external view returns (User[] memory) {
        ...
        else { if(adminAccess(msg.sender) || msg.sender == creator) return shareholders; }
        revert AccessDenied();
    }
